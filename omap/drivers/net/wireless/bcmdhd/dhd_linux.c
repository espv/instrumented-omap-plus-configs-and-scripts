...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
    ...
    // trace_sepext_srventry(dhd_start_xmit); // OYSTEDAL
    // Do not trace this location.
    // This function is called through the dev->dev_hard_start_xmit function
    // pointer, which we already trace from where it's called.
    ...
    // trace_sepext_pktqueue(skb->data, PKTQ_NIC_TX, 0, QUEUE_ENQUEUE); // OYSTEDAL
    ...
    // trace_sepext_srvexit(dhd_start_xmit); // OYSTEDAL
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
    ...
    trace_sepext_srventry(dhd_rx_frame); // OYSTEDAL
	...
	dhd_htsf_addrxts(dhdp, pktbuf);
        ...
        trace_sepext_pktextract((unsigned char *) skb, skb->truesize, 1);
    ...
    trace_sepext_srvexit(dhd_rx_frame); // OYSTEDAL
}
...
dhd_dpc_thread(void *data)
{
	...
	/* Run until signal received */
	while (1) {
        // TODO OYSTEDAL: Should this be modeled as an actual loop?
        trace_sepext_srventry(dhd_dpc_thread);
		if (down_interruptible(&tsk->sema) == 0) {
			...
			if (dhd->pub.busstate != DHD_BUS_DOWN) {
#if 1
                // OYSTEDAL: Rewritten to extract condition
                bool condition = dhd_bus_dpc(dhd->pub.bus);
                trace_sepext_queuecondition(!condition, PKTQ_NIC, PKTQ_NIC);
                if (condition) {
#else
			...
			break;
        trace_sepext_srvexit(dhd_dpc_thread);
	}
...
dhd_dpc(ulong data)
{
    ...
    trace_sepext_srventry(dhd_dpc); // OYSTEDAL
