...
#include <trace/sepextract.h> // OYSTEDAL
...
dhdsdio_txpkt(dhd_bus_t *bus, void *pkt, uint chan, bool free_pkt)
{
	...
	*(((uint16*)frame) + 1) = htol16(~len);
	...
	trace_sepext_pktextract(pkt, 0, 2);

    // OYSTEDAL
    // Approximated enqueueing location
	trace_sepext_pktqueue((void *) PKTDATA(osh, pkt), PKTQ_NIC, 0, QUEUE_ENQUEUE);
...
dhd_bus_txdata(struct dhd_bus *bus, void *pkt)
{
	...
	if (dhd_deferred_tx || bus->fcstate || pktq_len(&bus->txq) || bus->dpc_sched ||
        ...
        // OYSTEDAL
        // dhd_deferred_tx define to 1 above
        // TODO: This doesn't cover the condition properly
		trace_sepext_queuecondition(pktq_len(&bus->txq) == 0, PKTQ_BCM4329_TXQ, PKTQ_BCM4329_TXQ);
...
static uint
dhdsdio_sendfromq(dhd_bus_t *bus, uint maxframes, bool rxdone) // TODO: OYSTEDAL: rxdone to propagate state
{
	void *pkt;
	uint32 intstatus = 0;
	uint retries = 0;
    ...
    // TODO: OYSTEDAL: rxdone determines no. of tx's
	trace_sepext_queuecondition(rxdone, PKTQ_NIC, PKTQ_NIC);

	tx_prec_map = ~bus->flowcontrol;

    // 
	trace_sepext_loopstart(maxframes, 0, PKTQ_BCM4329_TXQ, PKTQ_BCM4329_TXQ);

	/* Send frames until the limit or some other event */
	for (cnt = 0; (cnt < maxframes) && DATAOK(bus); cnt++) {
		dhd_os_sdlock_txq(bus->dhd);

        trace_sepext_looprestart(0);
        ...
        // OYSTEDAL: Dequeue from tx queue
		trace_sepext_pktqueue(((struct sk_buff *) pkt)->data, PKTQ_BCM4329_TXQ, 0, QUEUE_DEQUEUE);
...
dhdsdio_readframes(dhd_bus_t *bus, uint maxframes, bool *finished)
{
    ...
    trace_sepext_srventry(dhdsdio_readframes); // OYSTEDAL
	...
	*finished = FALSE;
    ...
    trace_sepext_pktextract((void *) 0, 0, 4);

    // OYSTEDAL: trace_sepext_loopstart
	trace_sepext_loopstart(maxframes, 0, PKTQ_NIC, PKTQ_NIC);

	for (rxseq = bus->rx_seq, rxleft = maxframes;
	     !bus->rxskip && rxleft && bus->dhd->busstate != DHD_BUS_DOWN;
	     rxseq++, rxleft--) {

        trace_sepext_pktextract((void *) 0, 0, 0);

        // OYSTEDAL
        trace_sepext_looprestart(0);
#ifdef DHDTHREAD
#ifndef CONFIG_BCMDHD_DISABLE_EARLY_TX
        int hastxpkts = pktq_mlen(&bus->txq, ~bus->flowcontrol);
        trace_sepext_queuecondition(hastxpkts > 0, PKTQ_BCM4329_TXQ, PKTQ_BCM4329_TXQ);
		if ((bus->clkstate == CLK_AVAIL) && !bus->fcstate &&
			hastxpkts && DATAOK(bus)) {
			dhdsdio_sendfromq(bus, dhd_txbound, *finished);
		}
      	// OYSTEDAL: This is the original code. It was modified to extract and
		// and capture a state variable
		...
		*/
#endif /* CONFIG_BCMDHD_DISABLE_EARLY_TX */
#endif /* DHDTHREAD */
        ...
        // OYSTEDAL: Rewritten to capture state
        // TODO: Do we need this?
        // 23/10: This seems to always evaluate to false.
        bool readSingle = dhd_readahead && bus->nextlen;
        // trace_sepext_statecondition(STATE_SINGLE_READ, readSingle, 0, 0);
        if (readSingle) {
...
deliver:
		trace_sepext_pktqueue((void *) ((struct sk_buff*)(pkt))->data, PKTQ_NIC, 0, QUEUE_DEQUEUE);
    ...
    trace_sepext_srvexit(dhdsdio_readframes); // OYSTEDAL
...
clkwait:
	...
	trace_sepext_queuecondition(pktq_mlen(&bus->txq, ~bus->flowcontrol) == 0, PKTQ_BCM4329_TXQ, PKTQ_BCM4329_TXQ);
    // OYSTEDAL: I am not sure if DATAOK really signals if data is available...
    // DATAOK: A window is offered for data frames(?)
    // TXCTLOK: A window is offered control frames(?)
	// trace_sepext_statecondition(STATE_CTRL_AVAILABLE, TXCTLOK(bus), 0, 0);
	...
	/* Send queued frames (limit 1 if rx may still be pending) */
		...
		framecnt = dhdsdio_sendfromq(bus, framecnt, rxdone);
		txlimit -= framecnt;
	}
...
dhdsdio_isr(void *arg)
{
    ...
    trace_sepext_srventry(dhdsdio_isr); // OYSTEDAL
	...
	if (!bus) {
        ...
        trace_sepext_srvexit(dhdsdio_isr); // OYSTEDAL
		return;
	}
	sdh = bus->sdh;
