...
#if 1 // OYSTEDAL
    // We're trying to model an application doing actual work inside this spin,
    // which means that we should not be interruptible in this period.
    do {
        // ndelay wants at most 1 msec
...
static int pktgen_thread_worker(void *arg)
{
...
#if 0 // OYSTEDAL: We want migrations to be possible
	...
	while (!kthread_should_stop()) {
        // OYSTEDAL
        // This loop is (for modelling purposes) the same as an infinite loop,
        // so we model it as a service that is executed repeatedly by a thread.
        trace_sepext_srventry(pktgen_thread_worker);
		...
		if (unlikely(!pkt_dev && t->control == 0)) {
			...
			wait_event_interruptible_timeout(t->queue,
							 t->control != 0,
							 HZ/10);
            trace_sepext_srvexit(pktgen_thread_worker);
			try_to_freeze();
			continue;
		}
		...
		if (likely(pkt_dev)) {
            // OYSTEDAL: We're only interested in the cases where 
            trace_sepext_srventry(pktgen_xmit);
			pktgen_xmit(pkt_dev);
            trace_sepext_srvexit(pktgen_xmit);
...
// OYSTEDAL
static int __init pktgen_create_thread_migratable(void)
{
    static int kthreadid = 0;
	struct pktgen_thread *t;
	struct proc_dir_entry *pe;
	struct task_struct *p;
	...
	p = kthread_create(pktgen_thread_worker,
				   t,
				   "kpktgenmigrd_%d", kthreadid); // OYSTEDAL: migratable pktgen thread
	if (IS_ERR(p)) {
		pr_err("kthread_create() failed for kthreadid %d\n", kthreadid);
		list_del(&t->th_list);
		kfree(t);
		return PTR_ERR(p);
	}
...
#if 1 // OYSTEDAL
    int err;
    err = pktgen_create_thread_migratable();
    if (err)
			pr_warning("WARNING: Cannot create thread (%d)\n",
				   err);
#else
