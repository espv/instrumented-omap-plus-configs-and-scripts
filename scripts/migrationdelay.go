...
var migrate_pid = flag.Int("migrate", 0, "PID of thread to print migration events for")
...
func main() {
	...
	for scan.Scan() {
		...
		if entry[0] == "MIGRATE" {
			pid, err := strconv.ParseInt(entry[2], 10, 64)
			if err != nil {
				fmt.Println(err)
				continue
			}

			if pid != int64(*migrate_pid) {
				continue
			}

			fmt.Println("Migration at ", cycles)
			lastMigration = cycles
		}
		...
		case 2:
			if seqno != 0 {
				...
				if v, ok := packets[seqno]; ok {
					// TODO: Should possibly consider last migration before packet was received?
					fmt.Printf("DELTAMIGR(%d): %d %d\n", seqno, cycles-v, cycles-lastMigration)
